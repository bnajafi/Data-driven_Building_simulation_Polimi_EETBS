...
predict_DF_linearReg_CV = pd.DataFrame(predict_linearReg_CV, index =DF_target.index, columns = ["AC_consPredict_linearReg_CV"])
...
predict_DF_linearReg_CV_shorterPeriod = predict_DF_linearReg_CV["2014-08-01":"2014-08-03"]
predict_DF_linearReg_CV_shorterPeriod.plot()
plt.xlabel("time")
plt.ylabel("AC power")
plt.ylim([0,4000])
# Let's find the metrics !!
...
# Let's try a more complex algorithm, we will use only cross validation 
# It's called randomForest 
from sklearn.ensemble import RandomForestRegressor
reg_RF = RandomForestRegressor()
predict_RF_CV = cross_val_predict(reg_RF,DF_features,DF_target, cv=10)
predict_DF_RF_CV = pd.DataFrame(predict_RF_CV, index =DF_target.index, columns = ["AC_consPredict_RF_CV"])
predict_DF_RF_CV = predict_DF_RF_CV.join(DF_target)
predict_DF_RF_CV_ChosenDates = predict_DF_RF_CV["2014-08-01":"2014-08-20"]
predict_DF_RF_CV_ChosenDates.plot()
plt.xlabel("time")
plt.ylabel("AC power")
plt.ylim([0,4000])
plt.close("all")
predict_DF_RF_CV_shorterPeriod = predict_DF_RF_CV["2014-08-01":"2014-08-05"]
predict_DF_RF_CV_shorterPeriod.plot()
plt.xlabel("time")
plt.ylabel("AC power")
plt.ylim([0,4000])
# Let's find the metrics !!
